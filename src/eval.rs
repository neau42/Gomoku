...
fn check_pattern(test_priority: &mut HashMap<Priority, u8>, actual_priority: Priority) {
	if test_priority.contains_key(&actual_priority) {
		test_priority.insert(actual_priority.clone(), test_priority.get(&actual_priority).unwrap() + 1);
	} else {
		test_priority.insert(actual_priority, 1);
	}
}

pub fn evale_one_line(mut line: u64, test_priority: &mut HashMap<Priority, u8>, stone: u8) -> isize {
	let mut value = 0;
	let mut j: isize;
	...
	while line != 0 {
		match (line & 0b11_11_11_11_11_11) as u16 {
			...
			align5_white if (align5_white & 0b11_11_11_11_11 == WHITE_5_ALIGN) => {
				check_pattern(test_priority, Priority::WhiteWin);
				j = 10;
			},
			align5_black if (align5_black & 0b11_11_11_11_11 == BLACK_5_ALIGN) => {
				check_pattern(test_priority, Priority::BlackWin);
				j = 10;
			},
			FOUR_BLACK => {
				check_pattern(test_priority, Priority::BlackWin1);
				j = 10;
			},
			FOUR_WHITE => {
				check_pattern(test_priority, Priority::WhiteWin1);
				j = 10;
			},
			FOUR_BLACK_CLOSE1 |
			FOUR_BLACK_CLOSE2 |
			...
			FOUR_BLACK_CLOSE6 |
			FOUR_BLACK_CLOSE7 => {
				check_pattern(test_priority, Priority::BlackPossibleWin1);
				j = 8;
			},
			FOUR_WHITE_CLOSE1 |
			FOUR_WHITE_CLOSE2 |
			...
			FOUR_WHITE_CLOSE6 |
			FOUR_WHITE_CLOSE7 => {
				check_pattern(test_priority, Priority::WhitePossibleWin1);
				j = 8;
			},
			...
			align3black if (align3black & 0b00_11_11_11_11_11) == THREE_BLACK_OPEN => {// 10_000 
				check_pattern(test_priority, Priority::BlackPossibleWin2);
				j = 8;
			},
			align3black if (align3black & 0b00_11_11_11_11_11) == THREE_BLACK_OPEN_HOLE1
				...
				check_pattern(test_priority, Priority::BlackPossibleWin2Capturable);
				j = 8;
			},
			align3white if (align3white & 0b00_11_11_11_11_11) == THREE_WHITE_OPEN => {
				check_pattern(test_priority, Priority::WhitePossibleWin2);
				j = 8;
			},
			align3white if (align3white & 0b00_11_11_11_11_11) == THREE_WHITE_OPEN_HOLE1
				...
				check_pattern(test_priority, Priority::WhitePossibleWin2Capturable);
				j = 8;
			}
			_ => j = 2,
		}
...
pub fn eval(state: &mut Gameboard, actual_stone: u8, depth: u8, map_board_values: &mut HashMap<[u64; SIZE], isize>, player_stone: u8) {
	let mut score = if state.black_captures >= 10 {
		...
		*map_board_values.get(&state.cells).unwrap()
		...
		let mut test_priority: HashMap<Priority, u8> = HashMap::new();
		...
		let value = all.iter().map(|&e| evale_one_line(e, &mut test_priority, actual_stone)).sum();
