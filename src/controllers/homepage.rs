...
impl HomepageController {
    fn set_events(&mut self, widget_ids: &WidgetIds) {
        self.events.insert(widget_ids.dropdown_button_game_mode, Event::dropdown_button_game_mode(|model: &mut GameInfo, mode_index: Option<usize>| {
            model.set_mode(mode_index);
        }));

        self.events.insert(widget_ids.toggle_button_weight_boxes, Event::toggle_button_weight_boxes(|model: &mut GameInfo, display_weight: bool| {
            model.display_weight(display_weight);
        }));

        self.events.insert(widget_ids.number_dialer_first_ia_depth, Event::number_dialer_first_ia_depth(|model: &mut GameInfo, depth: f32| {
            model.set_first_ia_depth(depth);
        }));

        self.events.insert(widget_ids.number_dialer_second_ia_depth, Event::number_dialer_second_ia_depth(|model: &mut GameInfo, depth: f32| {
            model.set_second_ia_depth(depth);
        }));

        self.events.insert(widget_ids.button_start, Event::button_start(|model: &mut GameInfo| {
            model.change_window();
        }));
    }
}
